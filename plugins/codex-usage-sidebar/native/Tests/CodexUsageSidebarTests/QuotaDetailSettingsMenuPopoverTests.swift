...
final class QuotaDetailSettingsMenuPopoverTests: XCTestCase {
    ...
    func testHiddenMenuDoesNotRetainItsPreviousPointerRegion() {
        let popover = QuotaDetailSettingsMenuPopover()
        popover.show(
            relativeTo: CGRect(x: 620, y: 40, width: 30, height: 30),
            mode: .automatic,
        ...
        let parentPoint = CGPoint(
            x: popover.parentFrame!.midX,
            y: popover.parentFrame!.midY
        )
        let submenuPoint = CGPoint(
            x: popover.submenuFrame!.midX,
            y: popover.submenuFrame!.midY
        )

        XCTAssertTrue(popover.contains(parentPoint))
        XCTAssertTrue(popover.contains(submenuPoint))

        popover.hide()

        XCTAssertFalse(popover.contains(parentPoint))
        XCTAssertFalse(popover.contains(submenuPoint))
    }
}
