...
final class IndicatorInteractionViewTests: XCTestCase {
    func testHitTestingMatchesTheVisibleControlSurface() {
        let view = IndicatorInteractionView(
            frame: CGRect(x: 0, y: 0, width: 164, height: 46)
        )

        XCTAssertNil(view.hitTest(CGPoint(x: 82, y: 4)))
        XCTAssertIdentical(view.hitTest(CGPoint(x: 82, y: 23)), view)
        XCTAssertNil(view.hitTest(CGPoint(x: 82, y: 42)))
    }
