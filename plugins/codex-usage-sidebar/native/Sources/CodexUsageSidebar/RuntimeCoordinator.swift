...
final class RuntimeCoordinator: NSObject {
    ...
    func reconcileOverlay() {
        ...
        applyAutomaticFreeFallback()
        let automaticIndicatorFrame = contentHeader.collisionFreeIndicatorFrame ?? OverlayLayout.indicatorFrame(
            in: windowFrame,
        ...
        recordDiagnosticState(
            "shown placement=content-header mode=\(placementPreferences.mode.rawValue) anchor=\(anchor.source.rawValue) " +
    ...
    private func trackOverlayPosition() {
        ...
        overlay.reposition(
            to: resolvedIndicatorFrame(
                automaticFrame: contentHeader.collisionFreeIndicatorFrame ?? OverlayLayout.indicatorFrame(
                in: windowFrame,
                contentTrailingEdge: anchor.trailingEdge,
    ...
    private func applyAutomaticFreeFallback() {
        guard placementPreferences.mode == .automatic,
              contentHeader.shouldSwitchToFree,
              let frame = contentHeader.collisionFreeIndicatorFrame else { return }
        if let screen = placementStore.screen(containing: frame) {
            placementPreferences.switchToFreeFallback(
                frame: frame, visibleFrame: screen.visibleFrame,
                displayID: placementStore.displayID(for: screen)
            )
        } else {
            placementPreferences.mode = .free
        }
        placementStore.preferences = placementPreferences
        overlay.placementMode = .free
        recordDiagnosticState("placement=free reason=default-position-occupied")
    }
