...
final class OverlayPanel: NSObject {
    ...
    override init() {
        ...
        detailPanel.onLockOpenToggle = { [weak self] in
            self?.toggleLockedDetailOpen()
        }
...
extension OverlayPanel {
    ...
    private func pollHover() {
        ...
        let indicatorFrame = indicatorInteractionFrame
        let overIndicator = indicatorFrame.contains(point)
    ...
    private func toggleDetailPin() {
        ...
        detailInteraction.togglePinned(
            pointerInside: indicatorInteractionFrame.contains(
    ...
    private func toggleLockedDetailOpen() {
        guard isIndicatorVisible, latestDetail != nil else { return }
        let pointer = NSEvent.mouseLocation
        let pointerInside = panel.frame.contains(pointer) ||
            detailPanel.frame?.contains(pointer) == true
        detailInteraction.toggleLockedOpen(pointerInside: pointerInside)
        removePinnedDetailOutsideMonitors()
        updateControlAppearance()
        updateDetailVisibility()
    }
    ...
    private func updateDetailVisibility() {
            ...
            let latestDetail
        {
            detailPanel.show(
                ...
                relativeTo: panel.frame,
                theme: latestTheme,
                isLockedOpen: detailInteraction.isLockedOpen,
                lockAccessibilityLabel: detailInteraction.isLockedOpen
                    ? QuotaLocalization(language: latestLanguage).unlockDetailWindow
                    : QuotaLocalization(language: latestLanguage).lockDetailWindow
            )
        } else {
            detailSettingsMenu.hide()
            detailPanel.hide()
    ...
    private func dismissPinnedDetailIfPointerIsOutside() {
        ...
        let pointer = NSEvent.mouseLocation
        let isInsideIndicator = indicatorInteractionFrame.contains(pointer)
    ...
    private var indicatorInteractionFrame: CGRect {
        interactionView.interactionBounds.offsetBy(
            dx: panel.frame.minX,
            dy: panel.frame.minY
        )
    }
        ...
        appearance.performAsCurrentDrawingAppearance {
            switch OverlaySurfacePolicy.treatment(
                isIndicatorHovered: isHoveringIndicator ||
                    ...
                    detailInteraction.isLockedOpen
            ) {
            case .hostBackground:
