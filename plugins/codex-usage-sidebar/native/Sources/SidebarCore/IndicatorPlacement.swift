...
public struct IndicatorPlacementPreferences: Codable, Equatable, Sendable {
    ...
    public mutating func switchToFreeFallback(frame: CGRect, visibleFrame: CGRect, displayID: String) {
        guard mode == .automatic else { return }
        captureManualPlacement(frame: frame, visibleFrame: visibleFrame, displayID: displayID)
        mode = .free
    }
