...
if (-not [Runtime.InteropServices.RuntimeInformation]::IsOSPlatform([Runtime.InteropServices.OSPlatform]::Windows) -or
    ...
    throw 'Windows v0.3.0 setup verification requires Windows 11 AMD64/x64.'
}
...
$provenancePath = Join-Path $root 'WINDOWS-V030-PROVENANCE.json'
$checksumsPath = Join-Path $root 'WINDOWS-V030-SHA256SUMS.txt'
...
if ($provenance.schemaVersion -ne 1 -or
    ...
    $provenance.version -ne '0.3.0' -or
