...
version="0.3.5"
output_root="$repo_root/.dist/v0.3.5/macos"
app="$output_root/Codex Usage Sidebar Installer.app"
asset_name="codex-usage-sidebar-v0.3.5-macos-arm64.dmg"
dmg="$output_root/$asset_name"
checksums="$output_root/MACOS-V035-SHA256SUMS.txt"
provenance="$output_root/MACOS-V035-PROVENANCE.json"
stage_root=""

cleanup() {
  if [[ -n "$stage_root" && "$stage_root" == "${TMPDIR:-/tmp}"/cus-macos-v035-package.* ]]; then
...
[[ -d "$app" ]] || { printf 'macOS v0.3.5 installer app is missing\n' >&2; exit 66; }
...
[[ "$payload_commit" == "$source_commit" ]] || {
  printf 'macOS v0.3.5 installer payload commit differs from current source\n' >&2
  exit 67
}

stage_root="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/cus-macos-v035-package.XXXXXX")"
...
"$repo_root/scripts/verify-macos-v035-installer-package.sh" "$app" "$temporary_dmg"
