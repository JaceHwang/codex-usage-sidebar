...
verify_app() {
  ...
  [[ -d "$candidate" && -x "$executable" && -f "$manifest" && -f "$payload_provenance" \
    ...
    printf 'macOS v0.3.0 installer app is incomplete: %s\n' "$candidate" >&2
    exit 66
  }
...
[[ "$(/usr/bin/basename "$dmg")" == "$expected_asset" && -f "$dmg" ]] || {
  printf 'unexpected or missing macOS v0.3.0 DMG: %s\n' "$dmg" >&2
  exit 65
}
verify_app "$app"
/usr/bin/hdiutil verify "$dmg" >/dev/null
mount_root="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/cus-macos-v030-verify.XXXXXX")"
...
mount_root=""
printf 'PASS: exact macOS v0.3.0 arm64 installer app and DMG are valid on %s\n' "$required_branch"
