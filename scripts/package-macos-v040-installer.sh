...
version="0.4.0"
output_root="$repo_root/.dist/v0.4.0/macos"
app="$output_root/Codex Usage Sidebar Installer.app"
asset_name="codex-usage-sidebar-v0.4.0-macos-arm64.dmg"
dmg="$output_root/$asset_name"
checksums="$output_root/MACOS-V040-SHA256SUMS.txt"
provenance="$output_root/MACOS-V040-PROVENANCE.json"
stage_root=""

cleanup() {
  if [[ -n "$stage_root" && "$stage_root" == "${TMPDIR:-/tmp}"/cus-macos-v040-package.* ]]; then
...
[[ -d "$app" ]] || { printf 'macOS v0.4.0 installer app is missing\n' >&2; exit 66; }
...
[[ "$payload_commit" == "$source_commit" ]] || {
  printf 'macOS v0.4.0 installer payload commit differs from current source\n' >&2
  exit 67
}

stage_root="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/cus-macos-v040-package.XXXXXX")"
...
"$repo_root/scripts/verify-macos-v040-installer-package.sh" "$app" "$temporary_dmg"
...
sdk_version="$(/usr/bin/otool -l "$companion" | /usr/bin/awk '$1 == "cmd" && $2 == "LC_BUILD_VERSION" { inside=1; next } inside && $1 == "sdk" { print $2; exit }')"
[[ "$sdk_version" == 26.5 ]]
signature="adhoc"
