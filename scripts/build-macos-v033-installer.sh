...
cleanup() {
  if [[ -n "$stage_root" && "$stage_root" == "${TMPDIR:-/tmp}"/cus-macos-v033-build.* ]]; then
...
[[ ! -e "$final_app" ]] || {
  printf 'refusing to overwrite macOS v0.3.3 installer app: %s\n' "$final_app" >&2
  exit 66
}

stage_root="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/cus-macos-v033-build.XXXXXX")"
...
[[ -x "$rebuilt_executable" ]] || { printf 'rebuilt v0.3.3 companion is missing\n' >&2; exit 66; }
