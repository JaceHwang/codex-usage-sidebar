...
if ($LASTEXITCODE -ne 0 -or $branch -ne $requiredBranch) {
    throw "Windows v0.3.2 setup must be built from the exact '$requiredBranch' branch."
}
...
if ($LASTEXITCODE -ne 0 -or $porcelain.Length -ne 0) {
    throw 'Windows v0.3.2 setup requires a completely clean worktree, including untracked files.'
}
...
if ($evidenceRelative -ne $releaseDescriptor.evidencePath) {
    throw "Windows v0.3.2 setup requires the canonical '$ReleaseProfile' validation evidence path."
}
...
$deltaVerifier = Join-Path $repoRoot 'scripts\verify-v032-packaging-delta.py'
...
if ($LASTEXITCODE -ne 0) {
    throw "The Windows v0.3.2 '$ReleaseProfile' evidence gate failed."
}
...
$provenancePath = Join-Path $output 'WINDOWS-V032-PROVENANCE.json'
$checksumsPath = Join-Path $output 'WINDOWS-V032-SHA256SUMS.txt'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'The Windows v0.3.2 Host/Control publish failed.' }
    foreach ($required in @('CodexUsageSidebar.Windows.exe', 'CodexUsageSidebar.Control.exe')) {
        if (-not (Test-Path -LiteralPath (Join-Path $payload $required) -PathType Leaf)) {
            throw "The Windows v0.3.2 payload is missing $required."
        }
    }
    ...
    $manifestBuilder = Join-Path $repoRoot 'scripts\build-windows-v032-release-manifest.py'
    $payloadVerifier = Join-Path $repoRoot 'scripts\verify-windows-v032-release-payload.py'
    ...
    if ($LASTEXITCODE -ne 0) { throw 'The Windows v0.3.2 release manifest build failed.' }
    & python $payloadVerifier --release-profile $ReleaseProfile $payload
    if ($LASTEXITCODE -ne 0) { throw 'The Windows v0.3.2 release payload verification failed.' }
    ...
    if ($LASTEXITCODE -ne 0) { throw 'The embedded Windows v0.3.2 setup publish failed.' }
    ...
    if (-not (Test-Path -LiteralPath $builtSetup -PathType Leaf)) {
        throw 'The embedded Windows v0.3.2 setup output is incomplete.'
    }

    & $builtSetup --verify-embedded
    if ($LASTEXITCODE -ne 0) {
        throw 'The Windows v0.3.2 setup failed its embedded payload self-verification.'
    }
    ...
    $setupVerifier = Join-Path $repoRoot 'scripts\verify-windows-v032-setup.ps1'
    & $setupVerifier `
        ...
        -ReleaseProfile $ReleaseProfile
    if ($LASTEXITCODE -ne 0) {
        throw 'The copied Windows v0.3.2 setup candidate failed final verification.'
    }
