...
asset_name="codex-usage-sidebar-v0.3.2-macos-arm64.dmg"
dmg="$output_root/$asset_name"
checksums="$output_root/MACOS-V032-SHA256SUMS.txt"
provenance="$output_root/MACOS-V032-PROVENANCE.json"
stage_root=""

cleanup() {
  if [[ -n "$stage_root" && "$stage_root" == "${TMPDIR:-/tmp}"/cus-macos-v032-package.* ]]; then
...
[[ "$payload_commit" == "$source_commit" ]] || {
  printf 'macOS v0.3.2 installer payload commit differs from current source\n' >&2
  exit 67
}

stage_root="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/cus-macos-v032-package.XXXXXX")"
...
"$repo_root/scripts/verify-macos-v032-installer-package.sh" "$app" "$temporary_dmg"
