...
payload_ref="${CUS_RELEASE_SOURCE_COMMIT:-HEAD}"
installer_signing_identity="${CUS_INSTALLER_SIGN_IDENTITY:--}"
stage_root=""

usage() {
  printf 'usage: %s [--catalog PATH] [--source-commit REV]\n' "$0" >&2
}

while [[ "$#" -gt 0 ]]; do
  case "$1" in
    --catalog) catalog="$2"; shift 2 ;;
    --source-commit) payload_ref="$2"; shift 2 ;;
    *) usage; exit 64 ;;
  esac
done

cleanup() {
  if [[ -n "$stage_root" && "$stage_root" == "${TMPDIR:-/tmp}"/cus-macos-platform-build.* ]]; then
...
read -r version tag output_relative < <(/usr/bin/python3 - "$catalog" <<'PY'
import json
import sys

catalog = json.load(open(sys.argv[1], encoding="utf-8"))
release = catalog["planned"]["macos"]
print(release["version"], release["tag"], f".dist/{release['tag']}/macos")
PY
)
...
[[ ! -e "$final_app" ]] || {
  printf 'refusing to overwrite macOS release installer app: %s\n' "$final_app" >&2
  exit 66
}

stage_root="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/cus-macos-platform-build.XXXXXX")"
...
[[ "$plugin_version" == "$version" ]] || {
  printf 'macOS release requires plugin version %s, found %s\n' "$version" "$plugin_version" >&2
  exit 65
}
...
[[ -x "$rebuilt_executable" ]] || { printf 'rebuilt companion is missing\n' >&2; exit 66; }
...
/usr/bin/python3 - "$source_root/plugins/codex-usage-sidebar/assets/PROVENANCE.json" \
  "$source_commit" "$companion_sha" "$cdhash_sha" "$companion_signature" "$tag" <<'PY'
import json
import sys

output, commit, executable_sha, cdhash_sha, signature, tag = sys.argv[1:]
with open(output, "w", encoding="utf-8") as handle:
    json.dump({
        "schemaVersion": 1,
        "build": {"kind": "platform-release-asset", "tag": tag},
        "sourceCommit": commit,
        "companion": {
...
DEVELOPER_DIR="${DEVELOPER_DIR:-/Applications/Xcode.app/Contents/Developer}" \
  /usr/bin/xcrun swift build --package-path "$native_root" --configuration release --arch arm64 \
...
printf 'Built %s from source commit %s for %s\n' "$final_app" "$source_commit" "$tag"
