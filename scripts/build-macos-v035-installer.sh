...
payload_ref="${CUS_V035_SOURCE_COMMIT:-HEAD}"
installer_signing_identity="${CUS_INSTALLER_SIGN_IDENTITY:--}"
stage_root=""

cleanup() {
  if [[ -n "$stage_root" && "$stage_root" == "${TMPDIR:-/tmp}"/cus-macos-v035-build.* ]]; then
...
[[ ! -e "$final_app" ]] || {
  printf 'refusing to overwrite macOS v0.3.5 installer app: %s\n' "$final_app" >&2
  exit 66
}

stage_root="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/cus-macos-v035-build.XXXXXX")"
...
[[ "$plugin_version" == "$version" ]] || {
  printf 'macOS v0.3.5 release requires plugin version %s, found %s\n' "$version" "$plugin_version" >&2
  exit 65
}
...
[[ -x "$rebuilt_executable" ]] || { printf 'rebuilt v0.3.5 companion is missing\n' >&2; exit 66; }
...
with open(output, "w", encoding="utf-8") as handle:
    json.dump({
        "schemaVersion": 1,
        "build": {"kind": "v0.3.5-release-asset"},
        "sourceCommit": commit,
        "companion": {
