...
cleanup() {
  if [[ -n "$stage_root" && "$stage_root" == "${TMPDIR:-/tmp}"/cus-macos-platform-package.* ]]; then
...
read -r version tag asset_name checksums_name provenance_name < <(/usr/bin/python3 - "$catalog" <<'PY'
import json
import sys

release = json.load(open(sys.argv[1], encoding="utf-8"))["planned"]["macos"]
assets = release["assets"]
print(release["version"], release["tag"], assets["installer"], assets["checksums"], assets["provenance"])
PY
)
output_root="$repo_root/.dist/$tag/macos"
app="$output_root/Codex Usage Sidebar Installer.app"
dmg="$output_root/$asset_name"
checksums="$output_root/$checksums_name"
provenance="$output_root/$provenance_name"

for target in "$dmg" "$checksums" "$provenance"; do
  [[ ! -e "$target" ]] || { printf 'refusing to overwrite macOS release asset: %s\n' "$target" >&2; exit 66; }
done
[[ -d "$app" ]] || { printf 'macOS release installer app is missing\n' >&2; exit 66; }
...
[[ "$payload_commit" == "$source_commit" ]] || {
  printf 'installer payload commit differs from current source\n' >&2
  exit 67
}

stage_root="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/cus-macos-platform-package.XXXXXX")"
...
/usr/bin/hdiutil create -volname "Codex Usage Sidebar $tag" -srcfolder "$image_source" \
  -ov -format UDZO "$temporary_dmg" >/dev/null
/usr/bin/hdiutil verify "$temporary_dmg" >/dev/null
"$repo_root/scripts/verify-macos-platform-installer-package.sh" --catalog "$catalog" "$app" "$temporary_dmg"
...
/usr/bin/python3 - "$provenance" "$asset_name" "$dmg_sha" "$installer_sha" \
  "$companion_sha" "$source_commit" "$signature" "$sdk_version" "$version" "$tag" <<'PY'
import json
import sys

output, asset, dmg_sha, installer_sha, companion_sha, source, signature, sdk, version, tag = sys.argv[1:]
