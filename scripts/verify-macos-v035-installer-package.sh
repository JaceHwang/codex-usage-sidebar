...
dmg="$2"
version="0.3.5"
expected_asset="codex-usage-sidebar-v0.3.5-macos-arm64.dmg"
mount_root=""
...
verify_app() {
  ...
  [[ -d "$candidate" && -x "$executable" && -f "$manifest" && -f "$payload_provenance" \
    ...
    printf 'macOS v0.3.5 installer app is incomplete: %s\n' "$candidate" >&2
    exit 66
  }
...
[[ "$(/usr/bin/basename "$dmg")" == "$expected_asset" && -f "$dmg" ]] || {
  printf 'unexpected or missing macOS v0.3.5 DMG: %s\n' "$dmg" >&2
  exit 65
}
verify_app "$app"
/usr/bin/hdiutil verify "$dmg" >/dev/null
mount_root="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/cus-macos-v035-verify.XXXXXX")"
...
mount_root=""
printf 'PASS: exact macOS v0.3.5 arm64 installer app and DMG are valid\n'
