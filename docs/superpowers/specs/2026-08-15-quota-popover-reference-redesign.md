...
- Use the reference hierarchy: avatar/title/version at the top, large remaining percentage, progress track, remaining summary, token usage section, detailed quota rows, then the Jace/help footer.
- Match the reference card's wide-but-tall proportion, larger detail-row rhythm, and footer divider.
...
- Keep the exact localized Tibo link implementation available for the surrounding plugin surface; the approved reference card itself does not render the Tibo row.
...
- Keep the Tibo link implementation localized and tested, but omit its row from the approved reference card so the card content matches the supplied image.
...
1. A local runtime screenshot in Dark Aqua visually matches the supplied reference hierarchy, materials, colors, typography scale, separators, spacing, and Jace/question footer content.
...
4. Long Bank lists scroll inside the lower detail region without covering the token band or footer.
5. The Tibo implementation remains covered by its existing activation tests and is not part of the approved reference card geometry.
