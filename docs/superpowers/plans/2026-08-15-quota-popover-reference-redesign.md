...
**Goal:** Rebuild the native quota popover around the approved reference image, including the Jace/question footer, while preserving live data and existing placement behavior.
...
- Use the supplied reference image as the single visual source of truth; render the Jace footer and question-mark/help button.
...
- Preserve the Tibo URL implementation and dismiss-before-open ordering outside the approved reference card.
...
- Keep Bank details scrollable below the fixed token region and do not let scrolling overlap the footer.
...
**Interfaces:**
- Set a reference geometry (`width` approximately 476 points, `maximumHeight` approximately 804 points) while clamping against `visibleFrame`.
- Add `QuotaCardMaterialView` as the card's background layer, resolving material, border, and shadow colors during drawing.
- Keep `QuotaDetailLayout.informationFrames(in:tokenUsageVisible:)` as the only source for header, token band, scroll-area, and footer frames.

- [ ] **Step 1: Write failing geometry tests**

Assert the reference card stays inside the visible screen, the header/progress/token/detail/footer regions have the reference order, and the seven-slot token band has enough width for seven columns without a horizontal scroller.
...
Move the card background from a flat `windowBackgroundColor` layer to `QuotaCardMaterialView`, use the approved continuous corner radius and thin border, and keep the panel's nonactivating/pop-up behavior unchanged. Increase header/progress/token/detail/footer spacing together so the card does not merely stretch empty regions.

- [ ] **Step 4: Render a first visual fixture**

Run the existing visual fixture with `CUS_VISUAL_OUTPUT_DIR` and inspect one Dark Aqua and one Aqua image before proceeding to detail-row work. The fixture must show the Jace/question footer.
...
### Task 3: Match header, token chart, detail rows, and footer
...
- `QuotaDetailCardView` renders the larger title, avatar, version capsule, remaining percentage/summary, icon-led detail rows, and the Jace/question footer.

- [ ] **Step 1: Add failing visual/typography assertions**

Extend the fixture test to assert seven bar subviews, no horizontal scroller, the icon count matching detail rows, the footer help control, and the emphasized reset day/hour segments.
...
Use the existing semantic label colors and `QuotaColorScale`; render the reference avatar treatment, increase title/percentage typography, place the version capsule on the title baseline, add row icons/separators and the Jace/help footer. Make reset countdown digits visibly larger without changing Bank expiry segmentation.
...
Inspect Simplified Chinese, Traditional Chinese, and English in Aqua/Dark Aqua. Confirm no overlap, seven columns, footer alignment, readable long Bank rows, and the reference-like hierarchy.
...
Capture Dark Aqua and Aqua runtime screenshots. Verify the content-header indicator and popover placement, settings hiding, seven chart slots with zero future values, scrolling Bank rows, language fallback, and Jace/question footer alignment.
