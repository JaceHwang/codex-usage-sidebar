...
2026-08-22 将按钮文字两侧内边距调整为按钮高度的三分之一（源提交 `e4ea144`）：

- 保持按钮外部宽度 `132` 个逻辑像素和所有标题栏定位逻辑不变；默认按钮逻辑高度为 `28`，两侧内边距为 `28 / 3 ≈ 9.333` 个逻辑像素。
- 每次展示时根据实际按钮逻辑高度动态更新左右内边距，避免未来标题栏高度变化后比例失效。
- 实机 UIA 探针确认按钮范围为 `x=1493..1757`、文字区域为 `x=1513..1739`，右侧第一个原生按钮从 `x=1758` 开始；按钮外部宽度和右侧贴合关系均未改变。探针：`%TEMP%\codex-usage-sidebar-v0.3.2-probes\25-height-third-padding.json`。
- Windows solution 测试：`54 + 64 + 80 = 198/198` 通过；Release x64 构建 `0` 警告、`0` 错误。
...
2026-08-22 调整主题 logo 尺寸与文字间距（源提交 `2d04e75`）：

- logo 尺寸调整为 `20 × 20` 个逻辑像素，logo 与文字间距调整为 `4` 个逻辑像素；当前 200% DPI 下实测分别为 `40 × 40` 和 `8` 个物理像素。
- 按钮内容宽度同步改为 `124 + 20 + 4 = 148` 个逻辑像素；默认高度 `28` 时，总宽度为 `148 + 2 × (28 / 3) ≈ 166.667` 个逻辑像素，按钮文字不会被遮挡。
- 实机 UIA 探针确认插件窗口为 `x=1451..1784`、宽度 `333` 个物理像素；logo 为 `x=1478..1518`，文字从 `x=1526` 开始，间隔为 `8` 个物理像素；右侧第一个原生按钮从 `x=1785` 开始，右锚点仍保持 `1` 个物理像素间隔。探针：`%TEMP%\\codex-usage-sidebar-v0.3.2-probes\\28-logo-size-gap.json`。
- Windows solution 测试：`54 + 64 + 80 = 198/198` 通过；Release x64 构建 `0` 警告、`0` 错误。
