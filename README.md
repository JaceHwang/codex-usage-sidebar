...
| Windows 11 AMD64 (`x64`) | [v0.3.4 prerelease](https://github.com/JaceHwang/codex-usage-sidebar/releases/tag/v0.3.4) | Unsigned `x64` setup with signed compatibility updates; Windows ARM64 is out of scope |

Windows `v0.3.4` is the current Windows 11 AMD64/x64 prerelease line. It carries the macOS-visible quota card
...
and Windows CI preserve the stable macOS payload. The v0.3.4 Windows asset is a quick prerelease with
local package/runtime verification, provenance, and a signed compatibility-update pack; the new 85-case
Windows 11 x64 real-device matrix is still pending. The selector no longer rejects a Codex build
...
[Windows v0.3.4 prerelease notes](docs/releases/v0.3.4.md) and [Windows architecture notes](docs/ARCHITECTURE.md).
...
Windows ARM64 is not supported. The v0.3.4 setup uses explicit unsigned-install safeguards and must
...
1. Open the [v0.3.4 GitHub prerelease](https://github.com/JaceHwang/codex-usage-sidebar/releases/tag/v0.3.4) and download only
   `codex-usage-sidebar-v0.3.4-windows-x64-setup.exe` plus `WINDOWS-V034-SHA256SUMS.txt`.
2. Verify the setup SHA-256 before launching it:
   ...
   Compare it case-insensitively with the matching entry in `WINDOWS-V034-SHA256SUMS.txt`.
...
```text
Install Codex Usage Sidebar v0.3.4 from the GitHub Release on this Windows 11 AMD64/x64 machine.
Download codex-usage-sidebar-v0.3.4-windows-x64-setup.exe and WINDOWS-V034-SHA256SUMS.txt only,
...
- [v0.3.4 prerelease notes](docs/releases/v0.3.4.md)
