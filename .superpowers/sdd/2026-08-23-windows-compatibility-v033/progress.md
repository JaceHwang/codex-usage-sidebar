...
Task 6: fix round 1 (matrix compares literal titlebar geometry and routes English/Simplified-Chinese labels through the scanner; commit d866b97).
Task 6: fix round 2 (raw localized UIA names, per-DPI literal expected rectangles, and unresolved fixture through WindowsHostCoordinator to SafeDock; commit 7582bb3).
Task 6: automated scope complete (commits a8d8000..7582bb3, final fixture review clean). External release gate remains: no verified Windows device matrix record, so formal v0.3.3 installer stays non-publishable.
Task 6: partial real-device baseline (current source control diagnostic on Windows 11 x64, Codex build 151.0.7922.170, physical 200% DPI, default-redacted output and titlebar snapshot resolved; private evidence SHA-256 recorded in docs/validation/windows-v0.3.3-real-device-template.md; commit 2d7a712). This does not close any full matrix row.
